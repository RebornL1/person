...
上传包含多个sheet的Excel时，系统会自动识别sheet名称是否为日期格式：

| 支持格式 | 示例 | 说明 |
|----------|------|------|
| xx月xx日 | 4月21日 | 中文日期格式 |
| xxxx年xx月xx日 | 2026年4月21日 | 完整中文格式 |
| YYYY-MM-DD | 2026-04-21 | ISO日期格式 |
| MM-DD | 4-21 | 简短格式 |
| YYYYMMDD | 20260421 | 纯数字格式 |
| MMDD | 0423 | 无年份格式 |
| YYYY.MM.DD | 2026.04.21 | 点分隔格式 |
| Month Day | April 21 | 英文格式 |

### 多日数据汇总

当识别到多个日期sheet时：

1. **自动汇总**：按姓名聚合所有数值列（求和）
2. **平均值计算**：自动计算每个指标的日均值（如 `oncall接单未闭环的数量_平均`）
3. **出勤统计**：记录每人出现天数，识别全勤人员和出勤异常
4. **工作量分析优化**：多日汇总数据使用平均值计算工作量分，更公平反映日均负荷

### 空值处理

- 数值列空单元格（NaN、空字符串）自动按 `0` 处理
- 不影响汇总计算结果

### 示例文件

- `samples/test01.xlsx`：包含4月21日、4月22日、4月23日、4-24四个日期sheet
- `samples/multi_date_workload.xlsx`：包含4月21日、4月22日、4月23日三个日期sheet

---

## 智能风险预测功能

系统会自动分析多日数据，生成以下风险预测报告：

| 风险类型 | 分析逻辑 | 建议措施 |
|----------|----------|----------|
| 高压人员 | 工作量分超过平均值1.5倍 | 合理分配任务 |
| 透传异常 | 透传求助高于平均值2倍且产出较低 | 加强技能培训 |
| 出勤异常 | 出现天数少于3天 | 关注工作连续性 |
| 内核问题集中 | 内核问题占比超过50% | 专项根因分析 |
| 产出偏低 | 总产出低于平均值0.3倍 | 加强激励机制 |
...
4. **导入配置界面**（新功能）：
   - 查看日期识别状态（已识别/未识别标签）
   - 选择要导入的 Sheet 或选择「汇总所有日期」
   - 未识别日期会显示警告提示，可在导入后手动调整
   - 勾选需要的列（可多选）
...
5. 点击「开始导入」后，页面展示：
   ...
   - **智能风险预测报告**：高压人员、透传异常、出勤问题、内核问题集中等风险分析
   - **团队建议**：基于整体数据的改进建议
...
| 名称 | 文本 | 姓名，用于聚合 |
| 接单数工作量 | 数值 | 工作量总数 |
| 参会情况 | 文本 | 已参会/迟到/调休/请假等（标签展示） |
| 日期 | 文本 | 与sheet页对应 |
| 未闭环oncall接单数 | 数值 | 累加值，反映持续压力 |
| 名下待处理工单数 | 数值 | 累加值，反映持续压力 |
| 昨日新增工单数 | 数值 | 当日工作量衡量 |
| 咨询问题 | 数值 | 咨询类问题数量 |
| 内核技术支持 | 数值 | 内核问题（权重较高） |
| 管控技术支持 | 数值 | 管控问题 |
...
- **累加指标（正向）**：`未闭环oncall接单数`、`名下待处理工单数`
- **当日指标（正向）**：`昨日新增工单数`（权重较高）
- **技术支持（正向）**：`内核技术支持`（权重最高）、`管控技术支持`、`咨询问题`
- **负向指标**：`透传问题数量`、`不规范走单数量`
- **正向产出**：`昨日提单/需求数量`、`昨日案例总结数量`
- **派生指标**：`每日问题数量 = 咨询 + 内核支持 + 管控支持`
- **风险分**：基于 `透传问题 + 待处理工单 + 未闭环oncall` 计算

> 具体权重以 `models.py` 中 `DEFAULT_WEIGHTS` 为准，页面会直接展示当前模型公式。
